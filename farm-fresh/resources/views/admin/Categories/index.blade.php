        ...
        <div>
            <form method="get" action="/admin/categories/search">
                <div class="btn-group mx-2">
                    @csrf
                    <input class="form-control search-bar" type="search" name="search" placeholder="Search by name or id" value="{{ app('request')->input('search') }}" />
                    <button class="btn btn-success"><i class="fas fa-search"></i></button>
                </div>
            </form>

        </div>
        <button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#categoryModal" data-bs-whatever="Create"><i class="fa-solid fa-plus mx-1"></i>Create</button>
    </div>
</div>
                ...
                <tr>
                    ...
                    <td>
                        <div class="btn-group">
                            ...
                            <button type="button" class="btn btn-primary mx-2" id="edit_category" data-bs-toggle="modal" data-bs-target="#categoryModal" data-bs-whatever="Edit" data-bs-id="{{ $cat->id }}" data-bs-name="{{ $cat->name }}" data-bs-parent="{{ null == $cat->parent ? null : $cat->parent->id }}"><i class="fa-solid fa-pencil"></i></button>

                            <form method="post" action="{{ route('cat-delete', ['category' => $cat->id]) }}">
                                @csrf
                                @method('DELETE')
                                <input type="hidden" name="id" value="{{ $cat->id }}" />
                                <button class="btn btn-danger" onclick="return confirm('Are you sure you want to delete this category?')"><i class="fa-solid fa-trash-can"></i></button>
                            </form>
